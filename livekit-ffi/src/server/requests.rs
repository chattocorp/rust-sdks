...
fn on_capture_encoded_video_frame(
    ...
    Ok(proto::CaptureEncodedVideoFrameResponse { accepted: source.capture_encoded_frame(capture)? })
}

fn on_get_encoded_video_source_feedback(
    server: &'static FfiServer,
    request: proto::GetEncodedVideoSourceFeedbackRequest,
) -> FfiResult<proto::GetEncodedVideoSourceFeedbackResponse> {
...
pub fn handle_request(
    ...
    res.message = Some(match request {
        ...
        Request::GetEncodedVideoSourceFeedback(req) => {
            on_get_encoded_video_source_feedback(server, req)?.into()
        }
